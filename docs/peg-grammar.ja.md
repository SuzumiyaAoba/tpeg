...
上限のない繰り返し（`expr*`、`expr+`、上限を指定しない `expr{n,}`）で、
`expr` 自身が0文字にマッチしうる（nullable な）場合、その繰り返しには意味の
定まった挙動が存在しない：入力を1文字も消費しないまま「成功」し続けられて
しまうため、原理的な停止条件がないからである。コード生成系はこの形の規則を
生成時点で明確に拒否する（繰り返しがたまたまどう包まれているかという偶然の
事情によって挙動が変わるようなパーサを生成しない）。一方、範囲に上限がある
繰り返し（`expr{2,5}`、`expr{3,3}`）は `expr` が nullable かどうかに関わらず
意味が明確に定まるため、繰り返し回数そのものが上限として機能し、拒否される
ことはない。
...
いずれの形式も自己完結した検査であり、`&expr`/`!expr` は「ここで `expr` が
マッチするか」だけを問う。入力を消費することも、それ自体の外側に影響を
及ぼすこともない。`expr` の内部に書かれた `~`（カット/コミット演算子）は
`expr` 自身の試行にのみスコープが閉じており、先読みの境界で吸収される
- `&`/`!` 自体がどの選択の中にあろうと、それをコミットさせることはない。

## 演算子の優先順位

TPEG 自身の構文演算子は、優先順位の高い順に次のように結合する：

1. **一次式（Primary）** - リテラル、文字クラス、識別子、または括弧で
   囲んだ `(expr)` グループ（最も優先順位が高い）
2. **繰り返し（後置）** - `expr*`、`expr+`、`expr?`、`expr{n}`、
   `expr{n,m}`、`expr{n,}`
3. **先読み（前置）** - `&expr`、`!expr`
4. **ラベル** - `name:expr`
5. **連接** - `expr1 expr2 expr3`（並置）
6. **選択** - `expr1 / expr2 / expr3`（最も優先順位が低い）

繰り返しは先読みより*強く*結合する - `!expr*` は `(!expr)*` ではなく
`!(expr*)` と解釈される - これは標準的な PEG（Ford, POPL 2004:
`Prefix <- (AND / NOT)? Suffix`）や主要な PEG 実装（PEG.js、LPeg、Pest）と
一致する挙動である。逆の解釈を意図する場合は、明示的にグループ化すればよい：

```tpeg
!"a"*      // Not(Star("a")) -- 「`a` の連続にマッチしない」
(!"a")*    // Star(Not("a")) -- 「`a` にマッチしない位置が0個以上」
```

ラベルは先読みまでのすべてを包み込むため、`x:&"a"` と `x:!"a"*` は
それぞれ `x:(&"a")` と `x:(!("a"*))` になる - 「ラベル付き先読み」という
独立した優先順位レベルは存在しない。
