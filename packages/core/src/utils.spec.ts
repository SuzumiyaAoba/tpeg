...
describe("unicodeLength", () => {
  ...
  it("should return correct length for string with emojis", () => {
    expect(unicodeLength("hello 🌍")).toBe(7);
  });
...
describe("advancePos", () => {
  it("should advance position correctly for single line string", () => {
    const pos = { offset: 0, column: 0, line: 1 };
    const result = advancePos("hello", pos);
    expect(result).toEqual({ offset: 5, column: 5, line: 1 });
  });

  it("should advance position correctly for multi-line string", () => {
    const pos = { offset: 0, column: 0, line: 1 };
    const result = advancePos("hello\nworld", pos);
    expect(result).toEqual({ offset: 11, column: 5, line: 2 });
  });

  it("should handle emojis correctly", () => {
    const pos = { offset: 0, column: 0, line: 1 };
    const result = advancePos("🌍", pos);
...
describe("extractValue", () => {
  ...
  it("should throw error for failure result", () => {
    ...
    expect(() => extractValue(failure)).toThrow("Parse failed: test error");
  });
});
