...
async function testBootstrapSimple() {
  ...
  try {
    ...
    const calculatorGrammar = readFileSync("../parser-sample/examples/calculator.tpeg", "utf-8");
    ...
    if (stage1Result.success) {
      ...
      console.log(`⏱️  Generation time: ${stage1Result.performance.generationTime.toFixed(2)}ms`);
    ...
    const jsonGrammar = readFileSync("../parser-sample/examples/json-lite.tpeg", "utf-8");
    ...
    if (stage2Result.success) {
      ...
      console.log(`⏱️  Generation time: ${stage2Result.performance.generationTime.toFixed(2)}ms`);
      ...
      const improvementRatio = stage2Result.code.length / stage1Result.code.length;
    ...
    if (stage3Result.success) {
      ...
      console.log(`⏱️  Generation time: ${stage3Result.performance.generationTime.toFixed(2)}ms`);
      ...
      const hasAllRules = stage3Result.code.includes('complete_grammar') && 
      ...
      // Compare progression
      const stage1To3Ratio = stage3Result.code.length / stage1Result.code.length;
    ...
    const allStagesSuccess = stage1Result.success && stage2Result.success && stage3Result.success;
    const totalCodeGenerated = stage1Result.code.length + stage2Result.code.length + stage3Result.code.length;
    ...
    console.log("\n📋 Stage Comparison:");
    console.log(`   Stage 1 (Minimal): ${stage1Result.code.length} chars, ${stage1Result.performance.generationTime.toFixed(1)}ms`);
    console.log(`   Stage 2 (Extended): ${stage2Result.code.length} chars, ${stage2Result.performance.generationTime.toFixed(1)}ms`);
    console.log(`   Stage 3 (Complete): ${stage3Result.code.length} chars, ${stage3Result.performance.generationTime.toFixed(1)}ms`);
    
    // Final assessment
    console.log("\n🏆 Bootstrap Assessment:");
    if (allStagesSuccess) {
      console.log("🎉 EXCELLENT: Complete bootstrap sequence successful!");
      console.log("✅ TPEG can bootstrap itself from minimal to complete grammar");
    ...
    return {
      ...
      stages: [
        { id: 1, success: stage1Result.success, codeLength: stage1Result.code.length },
        { id: 2, success: stage2Result.success, codeLength: stage2Result.code.length },
        { id: 3, success: stage3Result.success, codeLength: stage3Result.code.length }
      ],
      totalTime,
      avgGenerationTime,
      totalCodeGenerated
