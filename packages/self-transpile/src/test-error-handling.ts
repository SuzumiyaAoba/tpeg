...
async function testErrorHandling() {
  ...
  for (let i = 0; i < scenarios.length; i++) {
    ...
    try {
      ...
      if (result.success) {
        console.log(
          `✅ Unexpected success: ${result.code.length} chars generated`,
        );
        ...
        console.log(
          `   🔄 Recovery attempted: ${errorMessages.includes("recovery") ? "Yes" : "No"}`,
        );
  ...
  console.log(
    `📈 Average time per test: ${(totalTime / scenarios.length).toFixed(2)}ms`,
  );
  ...
  if (successRate >= 80) {
    ...
    console.log(
      "⚠️  NEEDS IMPROVEMENT: Error handling system requires attention",
    );
  }
}
