...
async function testGeneratedCode() {
  ...
  try {
    ...
    if (result.success) {
      ...
      const functions = lines.filter(line => line.includes('const self_')).length;
      ...
      const expectedParsers = [
        'self_grammar', 'self_rule_list', 'self_rule_definition',
        'self_expression', 'self_choice_expr', 'self_sequence_expr',
        'self_primary_expr', 'self_string_literal', 'self_character_class',
        'self_identifier', 'self_number'
      ];
      ...
      console.log(`🔄 Recursion handling: ${hasRecursion ? '✅ Detected' : '❌ Not found'}`);
      ...
      const endSection = result.code.substring(Math.max(0, result.code.length - 500));
