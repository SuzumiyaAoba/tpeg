...
async function runPerformanceBenchmark() {
  ...
  console.log(
    `📊 Baseline Result: ${baselineResult.executionTime.toFixed(2)}ms, ${baselineResult.throughput.toFixed(0)} chars/sec`,
  );
  ...
  console.log(
    `📊 Optimized Result: ${optimizedResult.executionTime.toFixed(2)}ms, ${optimizedResult.throughput.toFixed(0)} chars/sec`,
  );

  const optimizationGain =
    ((baselineResult.executionTime - optimizedResult.executionTime) /
  ...
  for (const test of scalingTests) {
    ...
    const scalingResult = await benchmarkSingleTest(test.name, scalingGrammar, {
      targetLanguage: "typescript",
      includeTypes: true,
      optimize: true,
    ...
    console.log(
      `📊 ${test.name}: ${scalingResult.executionTime.toFixed(2)}ms, ${scalingResult.throughput.toFixed(0)} chars/sec`,
    );
  }
  ...
  const avgIterationTime =
    iterationResults.reduce((sum, r) => sum + r.time, 0) / iterations;
  ...
  console.log(
    `   🔄 Success rate: ${iterationResults.filter((r) => r.success).length}/${iterations}`,
  );
  ...
  const memoryTests = [
    {
      ...
      config: { optimize: true, enableMemoization: false },
    },
    {
      name: "Performance Optimized",
      config: { optimize: true, enableMemoization: true },
    },
    {
      name: "Full Features",
      config: {
  ...
  for (const test of memoryTests) {
    ...
    console.log(
      `📊 ${test.name}: ${memoryResult.peakMemory.toFixed(0)} bytes peak, ${memoryResult.executionTime.toFixed(2)}ms`,
    );
  }
  ...
  console.log(
    `🚀 Code generation rate: ${metrics.codeGenerationRate.toFixed(0)} chars/sec`,
  );
...
function generateScalingGrammar(ruleCount: number): string {
  ...
  for (let i = 0; i < ruleCount; i++) {
    if (i === 0) {
      ...
      rules.push(
        `  rule${i} = "part${i}" rule${i + 1}? ("," rule${(i + 2) % ruleCount})?`,
      );
    }
  }
...
function analyzePerformanceMetrics(
  ...
  const averageTime =
    successfulTests.length > 0 ? totalTime / successfulTests.length : 0;
  ...
  const memoryEfficiency =
    totalMemory > 0 ? (totalCodeGenerated / totalMemory) * 100 : 0;
  const codeGenerationRate =
    totalTime > 0 ? totalCodeGenerated / (totalTime / 1000) : 0;
  ...
  const iterationVariance =
    iterationResults.length > 0
      ? Math.max(...iterationResults.map((r) => r.time)) -
        Math.min(...iterationResults.map((r) => r.time))
      : 0;
...
function generateOptimizationRecommendations(
  ...
  if (metrics.averageTime > 30) {
    recommendations.push(
      "Enable code generation caching for repeated operations",
    );
  ...
  const hasLargeGrammarSlowdown = results.some(
    (r) => r.inputSize > 1000 && r.executionTime > 100,
  );
