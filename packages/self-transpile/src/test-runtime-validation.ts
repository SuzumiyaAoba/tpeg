...
`;
    ...
    console.log(
      `📊 Found ${exportedParsers.length} exported parser functions:`,
    );
    ...
    const hasValidImports =
      result.code.includes("import") && result.code.includes("tpeg-core");
    ...
    console.log(
      `   🔄 Recursion handling: ${hasRecursionComments ? "✅" : "❌"}`,
    );
    ...
    console.log(
      `🏆 Status: ${validationScore >= 4 ? "EXCELLENT" : validationScore >= 3 ? "GOOD" : "NEEDS IMPROVEMENT"}`,
    );
  } catch (error) {
