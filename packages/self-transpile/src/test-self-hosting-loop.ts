...
async function testSelfHostingLoop() {
  ...
  try {
      ...
      try {
        ...
        if (i > 0) {
          ...
          if (prevIteration) {
            ...
            console.log(`   📏 Length: ${lengthChange >= 0 ? '+' : ''}${lengthChange} chars`);
            console.log(`   ⏱️  Time: ${timeChange >= 0 ? '+' : ''}${timeChange.toFixed(2)}ms`);
          }
        }
    ...
    const avgGenerationTime = successfulIterations.length > 0 
      ? successfulIterations.reduce((sum, it) => sum + it.generationTime, 0) / successfulIterations.length
      : 0;
    
    const finalCodeLength = successfulIterations.length > 0 
      ? (successfulIterations[successfulIterations.length - 1]?.codeLength ?? 0)
      : 0;
    
    const lastIteration = successfulIterations[successfulIterations.length - 1];
    const secondLastIteration = successfulIterations[successfulIterations.length - 2];
    const stabilityAchieved = Boolean(converged || (
      successfulIterations.length >= 2 && 
      lastIteration && 
      secondLastIteration &&
      Math.abs(lastIteration.codeLength - secondLastIteration.codeLength) < 10
    ));
    ...
    console.log(`🎯 Converged: ${converged ? `✅ (iteration ${convergenceIteration})` : '❌'}`);
    ...
    iterations.forEach(it => {
      ...
      console.log(`   ${it.iteration}: ${status} ${it.generationTime.toFixed(1)}ms, ${it.codeLength} chars${convergence}`);
...
testSelfHostingLoop()
  .then(result => {
    ...
    console.log(`🔄 Final status: ${result.converged ? 'CONVERGED' : 'STABLE'}`);
  })
  .catch(console.error); 