...
import { selfTranspile, createSelfTranspileContext } from "./self-transpile";
import type { BootstrapConfig, BootstrapResult, SelfTranspileContext } from "./types";
...
async function validateBootstrapProcess(config: BootstrapConfig): Promise<BootstrapResult> {
  ...
  try {
    ...
    const stages: BootstrapStage[] = [
      ...
      },
      {
        ...
        validationCriteria: {
          minCodeLength: 1500,
          maxCodeLength: 3500,
          requiredExports: ["grammar", "expression", "choice_expr", "repetition"],
      ...
      },
      {
        ...
        inputGrammar: readFileSync("../parser-sample/examples/tpeg-self.tpeg", "utf-8"),
        expectedFeatures: ["complete_syntax", "self_hosting"],
        validationCriteria: {
          minCodeLength: 2500,
          maxCodeLength: 4000,
          requiredExports: ["grammar", "rule_definition", "expression", "string_literal", "character_class"],
    ...
    for (let i = 0; i < stages.length; i++) {
      ...
      while (attemptCount < 3 && !stageSuccess) {
        ...
        try {
          ...
          if (validation.allPassed) {
            ...
            console.log(`❌ Validation failed: ${validation.failedChecks.join(", ")}`);
            if (attemptCount === 3) {
              // Rollback attempt
      ...
      const stageResult: StageResult = {
        ...
        validationResults: stageSuccess ? 
          ...
          { lengthCheck: false, exportsCheck: false, featuresCheck: false, structureCheck: false },
      ...
      if (stageSuccess && generatedCode) {
        ...
        currentContext.generatedFunctions.set(`stage_${stage.id}`, generatedCode);
    ...
    const finalValidation = await performFinalValidation(stageResults, config);
    ...
    stageResults.forEach((result, index) => {
      ...
      console.log(`   Stage ${index + 1}: ${status} ${result.stage.name} (${result.generationTime.toFixed(1)}ms)`);
    });
    
    const bootstrapResult: BootstrapResult = {
      parserCode: completedStages > 0 ? stageResults[completedStages - 1].generatedCode : "",
      ...
      tests: config.generateTests ? generateBootstrapTests(stageResults) : undefined
    };
...
function validateStageOutput(stage: BootstrapStage, code: string): any {
  const lengthCheck = code.length >= stage.validationCriteria.minCodeLength && 
                    code.length <= stage.validationCriteria.maxCodeLength;
  
  const exportsCheck = stage.validationCriteria.requiredExports.every(
    exportName => code.includes(`export const ${stage.name.toLowerCase().replace(/\s+/g, '_')}_${exportName}`) ||
  ...
  const structureCheck = code.includes('import') && code.includes('export') && code.includes('Parser');
  
  const allPassed = lengthCheck && exportsCheck && featuresCheck && structureCheck;
  const failedChecks = [
    !lengthCheck && 'length',
...
async function performFinalValidation(stageResults: StageResult[], config: BootstrapConfig) {
  console.log("🔍 Performing final validation...");
  
  // Check if we can parse the original grammar
  const canParseOriginal = stageResults.length > 0 && stageResults[stageResults.length - 1].success;
  
  // Check consistency
  const canGenerateConsistent = stageResults.filter(r => r.success).length >= 2;
  
  // Check performance
  const avgTime = stageResults.reduce((sum, r) => sum + r.generationTime, 0) / stageResults.length;
  ...
  console.log(`✅ Performance acceptable: ${performanceAcceptable} (${avgTime.toFixed(2)}ms avg)`);
