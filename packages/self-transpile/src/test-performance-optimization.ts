...
import { createOptimizedTranspiler, selfTranspileOptimized } from "./performance-optimization";
...
async function testPerformanceOptimization() {
  ...
  const comprehensiveResult = await testComprehensiveOptimization(grammarSource);
  ...
  const avgMemoryReduction = testResults.reduce((sum, r) => sum + r.memoryReduction, 0) / testResults.length;
  ...
  testResults.forEach((result, index) => {
    ...
    console.log(`      Memory: ${result.memoryReduction.toFixed(1)}% reduction`);
    console.log(`      Status: ${result.success ? '✅ Success' : '❌ Failed'}`);
  });
  
  // Performance Grade
  const performanceGrade = calculateOptimizationGrade(avgImprovement, avgMemoryReduction, successRate);
...
async function compareBasicPerformance(
  ...
  const memoryReduction = baselineMemory > 0 ? ((baselineMemory - optimizedMemory) / baselineMemory) * 100 : 0;
...
async function testCachingPerformance(grammarSource: string): Promise<PerformanceTestResult> {
  ...
  const result1 = await transpiler.transpile(grammarSource, { namePrefix: "cache_test_" });
  const time1 = performance.now() - start1;
  
  // Second call (cache hit)
  const start2 = performance.now();
  const result2 = await transpiler.transpile(grammarSource, { namePrefix: "cache_test_" });
...
async function testMemoryPoolingPerformance(grammarSource: string): Promise<PerformanceTestResult> {
  // Without memory pooling
  const baselineStart = performance.now();
  const baselineMemoryStart = process.memoryUsage().heapUsed;
  
  const baselineResult = await selfTranspileOptimized(grammarSource, {}, {
    enableCaching: false,
    enableMemoryPooling: false,
    enableParallelProcessing: false,
    enableLazyEvaluation: false,
  ...
  const optimizedResult = await selfTranspileOptimized(grammarSource, {}, {
    enableCaching: false,
    enableMemoryPooling: true,
    enableParallelProcessing: false,
    enableLazyEvaluation: false,
...
async function testParallelProcessingPerformance(grammarSource: string): Promise<PerformanceTestResult> {
...
async function testIterationPerformance(grammarSource: string): Promise<PerformanceTestResult> {
  const iterations = 10;
  ...
  const optimizedStart = performance.now();
  for (let i = 0; i < iterations; i++) {
    await selfTranspileOptimized(grammarSource, { namePrefix: `optimized_iter_${i}_` }, {
      enableCaching: true,
      enableMemoryPooling: true,
      enableParallelProcessing: true,
      enableLazyEvaluation: true,
...
async function testStringBuilderPerformance(grammarSource: string): Promise<PerformanceTestResult> {
...
async function testComprehensiveOptimization(grammarSource: string): Promise<PerformanceTestResult> {
...
function generatePerformanceRecommendations(results: PerformanceTestResult[]): string[] {
  ...
  if (failedTests.length > 0) {
    recommendations.push("Fix optimization failures in: " + failedTests.map(t => t.testName).join(", "));
  }
  
  if (avgImprovement >= 30) {
    recommendations.push("Consider implementing these optimizations in production");
  }
