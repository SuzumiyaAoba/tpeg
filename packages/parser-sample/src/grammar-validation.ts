...
async function validateGrammars(): Promise<ValidationResult[]> {
  ...
  try {
    ...
    for (const file of grammarFiles) {
      ...
      try {
        ...
        try {
          ...
          console.log(`      📦 Generated: ${generated.exports.length} exports`);
          ...
          console.log(`   ❌ Code generation failed: ${codegenError instanceof Error ? codegenError.message : String(codegenError)}`);
        }
        ...
        console.log(`   ❌ File error: ${fileError instanceof Error ? fileError.message : String(fileError)}`);
      }
    ...
    console.error(`❌ Failed to read examples directory: ${dirError instanceof Error ? dirError.message : String(dirError)}`);
  }
...
function generateSummary(results: ValidationResult[]): void {
  ...
  if (successful.length > 0) {
    ...
    for (const result of successful) {
      console.log(`   • ${result.grammar} (${result.rules} rules, ${result.annotations} annotations)`);
    }
  }
  ...
  const totalAnnotations = successful.reduce((sum, r) => sum + r.annotations, 0);
  
  console.log(`\n📈 Statistics:`);
  console.log(`   Total Rules: ${totalRules}`);
  console.log(`   Total Annotations: ${totalAnnotations}`);
  console.log(`   Average Rules per Grammar: ${(totalRules / successful.length || 0).toFixed(1)}`);
