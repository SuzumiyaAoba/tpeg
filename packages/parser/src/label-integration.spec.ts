...
import type { LabeledExpression, Sequence, Choice, Star, Plus, Optional, Group, PositiveLookahead, NegativeLookahead } from './types';
...
describe('Label Integration Tests', () => {
  ...
  describe('realistic grammar patterns with labels', () => {
    test('arithmetic expression with labels', () => {
      const parser = expression();
      const result = parser('left:[0-9]+ op:"+" right:[0-9]+', createPosition());
    ...
    test('function call pattern with labels', () => {
      const parser = expression();
      const result = parser('func:identifier "(" args:(expr ("," expr)*)? ")"', createPosition());
    ...
    test('assignment statement with labels', () => {
      const parser = expression();
      const result = parser('target:identifier "=" value:"hello"', createPosition());
  ...
  describe('edge cases and error conditions', () => {
    ...
    test('labels work with complex repetition patterns', () => {
      const parser = expression();
      const result = parser('items:(name:"hello" / count:[0-9]+){2,5}', createPosition());
