...
import { NamespaceManager, NamespaceConflictError, QualifiedNameResolutionError } from './namespace-manager.js';
...
describe('NamespaceManager', () => {
  ...
  // テストデータ作成ヘルパー
  const createRule = (name: string, pattern: any = { type: 'Identifier', name: 'test' }): RuleDefinition => ({
    type: 'RuleDefinition',
    name,
    pattern,
  });

  const createGrammar = (name: string, rules: RuleDefinition[]): GrammarDefinition => ({
  ...
  const createQualifiedId = (module: string, name: string): QualifiedIdentifier => ({
    type: 'QualifiedIdentifier',
    module,
    name,
  });
  ...
  describe('registerModule', () => {
    ...
    it('should register a module with namespace', () => {
      ...
      const moduleFile = createModuleFile('test.tpeg', [grammar], [], moduleInfo);
    ...
    it('should register a module with imports and exports', () => {
      const rule1 = createRule('rule1');
      const rule2 = createRule('rule2');
      const exports: ExportDeclaration = { type: 'ExportDeclaration', rules: ['rule1'] };
      const imports: ImportStatement[] = [
        { type: 'ImportStatement', modulePath: 'other.tpeg', alias: 'other' }
      ];
      const grammar = createModularGrammar('TestGrammar', [rule1, rule2], exports);
  ...
  describe('resolveQualifiedName', () => {
    beforeEach(() => {
      ...
      const mainModule = createModuleFile('main.tpeg', [mainGrammar], [
    ...
    it('should throw error for non-exported rule', () => {
      ...
      const privateGrammar = createModularGrammar('PrivateGrammar', [privateRule]);
      const privateModule = createModuleFile('private.tpeg', [privateGrammar]);
      manager.registerModule(privateModule);

      const mainModule = createModuleFile('main2.tpeg', [createGrammar('Main', [])], [
        { type: 'ImportStatement', modulePath: 'private.tpeg', alias: 'priv' }
      ]);
      manager.registerModule(mainModule);
  ...
  describe('checkNamespaceConflicts', () => {
    it('should detect namespace conflicts', () => {
      ...
      const mainModule = createModuleFile('main.tpeg', [mainGrammar], [
        { type: 'ImportStatement', modulePath: 'module1.tpeg', alias: 'mod1' },
        { type: 'ImportStatement', modulePath: 'module2.tpeg', alias: 'mod2' }
      ]);
      manager.registerModule(mainModule);
