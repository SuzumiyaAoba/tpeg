...
import { EtaTPEGCodeGenerator, generateEtaTypeScriptParser } from './eta-codegen';
import {
...
describe('EtaTPEGCodeGenerator', () => {
  ...
  describe('Performance Analysis', () => {
    ...
    it('should detect high complexity grammars', async () => {
      ...
      expect(result.performance.optimizationSuggestions.length).toBeGreaterThan(0);
      expect(result.performance.optimizationSuggestions.some(s => 
  ...
  describe('Combinator Collection', () => {
    it('should collect all necessary combinators', async () => {
      ...
      const combinatorImport = result.imports.find(imp => 
        imp.includes('from "tpeg-core"') && !imp.includes('type')
      );
