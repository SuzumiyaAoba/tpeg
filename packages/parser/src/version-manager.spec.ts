...
describe('VersionManager', () => {
  ...
  describe('compareVersions', () => {
    ...
    it('should compare prerelease versions', () => {
      ...
      const v2: SemanticVersion = { major: 1, minor: 0, patch: 0, prerelease: 'beta' };
  ...
  describe('satisfiesConstraint', () => {
    ...
    it('should handle caret constraint', () => {
      ...
      // Compatible versions
      expect(manager.satisfiesConstraint({ major: 1, minor: 2, patch: 3 }, constraint)).toBe(true);
      expect(manager.satisfiesConstraint({ major: 1, minor: 2, patch: 4 }, constraint)).toBe(true);
      expect(manager.satisfiesConstraint({ major: 1, minor: 3, patch: 0 }, constraint)).toBe(true);
      
      // Incompatible versions
      expect(manager.satisfiesConstraint({ major: 2, minor: 0, patch: 0 }, constraint)).toBe(false);
      expect(manager.satisfiesConstraint({ major: 1, minor: 2, patch: 2 }, constraint)).toBe(false);
    });
    ...
    it('should handle tilde constraint', () => {
      ...
      // Incompatible versions
      expect(manager.satisfiesConstraint({ major: 1, minor: 3, patch: 0 }, constraint)).toBe(false);
      expect(manager.satisfiesConstraint({ major: 1, minor: 2, patch: 2 }, constraint)).toBe(false);
    });
    ...
    it('should handle range constraints', () => {
      ...
      expect(manager.satisfiesConstraint(testVersion, constraints[0])).toBe(true); // >=
      expect(manager.satisfiesConstraint(testVersion, constraints[1])).toBe(false); // <=
      expect(manager.satisfiesConstraint(testVersion, constraints[2])).toBe(true); // >
      expect(manager.satisfiesConstraint(testVersion, constraints[3])).toBe(false); // <
    });

    it('should handle wildcard constraint', () => {
      const constraint: VersionConstraint = { operator: '*', version: { major: 0, minor: 0, patch: 0 } };
      expect(manager.satisfiesConstraint(version, constraint)).toBe(true);
      expect(manager.satisfiesConstraint({ major: 999, minor: 999, patch: 999 }, constraint)).toBe(true);
    });
  });
  ...
  describe('registerModule', () => {
    ...
    it('should register a module with specified version', () => {
      ...
      const moduleFile = createModuleFile('test.tpeg', [createGrammar('TestGrammar')], [], moduleInfo);
      manager.registerModule(moduleFile);
    ...
    it('should register a module with dependencies', () => {
      const imports: ImportStatement[] = [
        ...
        { type: 'ImportStatement', modulePath: 'utils.tpeg', version: '~2.0.0' }
      ];
      const moduleFile = createModuleFile('test.tpeg', [createGrammar('TestGrammar')], imports);
      manager.registerModule(moduleFile);
  ...
  describe('checkCompatibility', () => {
    ...
    it('should return false for invalid versions', () => {
      expect(manager.checkCompatibility('test', 'invalid', '1.0.0')).toBe(false);
      expect(manager.checkCompatibility('test', '^1.0.0', 'invalid')).toBe(false);
    });
  });
  ...
  describe('validateDependencies', () => {
    beforeEach(() => {
      // Setup test modules
      const baseModule = createModuleFile('base.tpeg', [createGrammar('BaseGrammar')], [], {
        type: 'ModuleInfo',
        version: '1.2.0'
      });
      manager.registerModule(baseModule);

      const utilsModule = createModuleFile('utils.tpeg', [createGrammar('UtilsGrammar')], [], {
        type: 'ModuleInfo',
        version: '2.1.0'
      });
    ...
    it('should validate compatible dependencies', () => {
      const mainModule = createModuleFile('main.tpeg', [createGrammar('MainGrammar')], [
        { type: 'ImportStatement', modulePath: 'base.tpeg', version: '^1.0.0' },
        { type: 'ImportStatement', modulePath: 'utils.tpeg', version: '~2.1.0' }
      ]);
    ...
    it('should throw error for incompatible dependencies', () => {
      const mainModule = createModuleFile('main.tpeg', [createGrammar('MainGrammar')], [
        { type: 'ImportStatement', modulePath: 'base.tpeg', version: '^2.0.0' } // incompatible
      ]);
      manager.registerModule(mainModule);
    ...
    it('should throw error for missing dependencies', () => {
      const mainModule = createModuleFile('main.tpeg', [createGrammar('MainGrammar')], [
        { type: 'ImportStatement', modulePath: 'nonexistent.tpeg', version: '^1.0.0' }
      ]);
      manager.registerModule(mainModule);
    ...
    it('should detect conflicting modules', () => {
      const conflictingModule = createModuleFile('conflicting.tpeg', [createGrammar('ConflictingGrammar')], [], {
        type: 'ModuleInfo',
        version: '1.0.0'
      });
  ...
  describe('utility methods', () => {
    beforeEach(() => {
      const module1 = createModuleFile('module1.tpeg', [createGrammar('Module1')], [
        { type: 'ImportStatement', modulePath: 'module2.tpeg', version: '^1.0.0' }
      ]);
      const module2 = createModuleFile('module2.tpeg', [createGrammar('Module2')]);
